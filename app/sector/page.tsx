...
function Pill({ label, value, bull }: { label: string; value: string; bull?: boolean | null }) {
  ...
  if (bull === true) color = 'var(--color-brand-bull)';
  if (bull === false) color = 'var(--color-brand-bear)';
...
function SectorCard({ sec }: { sec: SectorStrength }) {
  ...
  const intensity = Math.min(Math.abs(sec.strength) / 24, 1);
  const avgEdge = typeof sec.avgEdge === 'number' ? sec.avgEdge : 0;
  const bullRatio = typeof sec.bullRatio === 'number' ? sec.bullRatio : 0.5;
  const accent = isBull ? 'var(--color-brand-bull)' : 'var(--color-brand-bear)';
  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: `rgba(${isBull ? '5,217,143' : '240,54,90'},${0.12 + intensity * 0.35})`,
      }}
    >
      <div
      ...
      <div
        ...
        style={{
          background: `radial-gradient(circle at 85% 15%, rgba(${isBull ? '5,217,143' : '240,54,90'},${0.03 + intensity * 0.07}) 0%, transparent 65%)`,
        }}
      />
      ...
      <div className="relative p-4">
        <div className="flex justify-between items-start mb-3">
          <div>
            ...
            <div className="font-mono text-[9px] mt-0.5" style={{ color: 'var(--color-brand-muted)' }}>
              {sec.stocks.length} signal{sec.stocks.length !== 1 ? 's' : ''}
            </div>
          </div>
          <div
          ...
          >
            {isBull ? 'UP' : 'DN'} {sec.strength > 0 ? '+' : ''}{sec.strength.toFixed(1)}
          </div>
        </div>
        ...
        <div className="mb-3">
          <div className="flex justify-between font-mono text-[9px] mb-1" style={{ color: 'var(--color-brand-muted)' }}>
            <span>AVG EDGE</span>
            <span style={{ color: accent }}>{avgEdge > 0 ? '+' : ''}{avgEdge.toFixed(1)}</span>
          </div>
          <div className="h-1 rounded-full overflow-hidden" style={{ background: 'var(--color-brand-border)' }}>
            <div className="h-full rounded-full" style={{ width: `${Math.min(intensity * 100, 100)}%`, background: accent }} />
          </div>
        </div>

        <div className="flex h-1.5 rounded-full overflow-hidden mb-3" style={{ background: 'var(--color-brand-bearbg)' }}>
          <div style={{ width: `${bullRatio * 100}%`, background: 'var(--color-brand-bull)' }} />
        </div>
        <div className="flex justify-between font-mono text-[9px] mb-3" style={{ color: 'var(--color-brand-muted)' }}>
          <span style={{ color: 'var(--color-brand-bull)' }}>BULL {(bullRatio * 100).toFixed(0)}%</span>
          <span style={{ color: 'var(--color-brand-bear)' }}>BEAR {((1 - bullRatio) * 100).toFixed(0)}%</span>
        </div>

        {sec.stocks.length > 0 && (
          <div className="border-t pt-3 space-y-1.5" style={{ borderColor: 'rgba(26,40,64,0.5)' }}>
            {sec.stocks.slice(0, 4).map((stock, index) => {
              const edge = toNumber(stock.SignalEdge);
              const side = resolveSignalSide(stock as Record<string, unknown>);
              const sideColor = side === 'BEARISH' ? 'var(--color-brand-bear)' : 'var(--color-brand-bull)';

              return (
                <div key={`${stock.Name}-${index}`} className="flex justify-between items-center gap-2">
                  <span className="text-[11px] font-medium truncate" style={{ color: 'var(--color-brand-text)', opacity: 0.85 }}>
                    {stock.Name}
                  </span>
                  <span className="font-mono text-[10px] font-semibold ml-2 shrink-0" style={{ color: sideColor }}>
...
export default async function SectorPage({ searchParams }: { searchParams: DateSearchParams }) {
  ...
  const bullSectors = sectors.filter((sector) => sector.strength > 0).length;
  const bearSectors = sectors.filter((sector) => sector.strength < 0).length;
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold tracking-wide text-white">Sector Rotation</h1>
          </div>
          <p className="font-mono text-xs tracking-widest" style={{ color: 'var(--color-brand-muted)' }}>
            SIGNAL-BASED ROTATION MAP
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <Pill label="LEADER" value={topSector} />
      </div>
      ...
      {sectors.length === 0 ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {sectors.map((sector) => <SectorCard key={sector.name} sec={sector} />)}
          </div>
        </>
      )}
    </div>
