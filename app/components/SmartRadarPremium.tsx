          ...
          {topFive.map((stock, index) => (
            ...
            >
              ...
              <div className="mb-4">
                ...
                <div className="text-3xl font-black text-brand-accent">{stock.Peak_Score}</div>
              </div>

              <div className="mb-4">
                <div className="text-brand-muted text-xs">SMART RANK</div>
                <div className="text-lg font-mono text-white">{stock.SmartRank}</div>
              </div>

              <div className="mb-4 text-sm text-brand-muted">
                OI Change: <span className="text-white font-semibold">{stock.OI}%</span>
              </div>

              <div>
                <div className="flex justify-between text-xs text-brand-muted mb-1">
                  <span>CONFIDENCE</span>
                  <span>{stock.Signal_Generated_Score}%</span>
                </div>

                <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                  <div
                    className={`${confidenceColor(stock.Signal_Generated_Score)} h-full transition-all duration-500`}
                    style={{ width: `${Math.min(stock.Signal_Generated_Score, 100)}%` }}
                  />
                </div>
              </div>
            ...
            >
              ...
              <div className="flex items-center gap-6 text-sm">
                <div>
                  ...
                  <span className="text-white">{stock.Peak_Score}</span>
                </div>

                <div>
                  <span className="text-brand-muted">Rank:</span>{' '}
                  <span className="text-white">{stock.SmartRank}</span>
                </div>
