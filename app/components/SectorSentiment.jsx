...
const MOCK = [
  { sector: 'BANKING', bull_count: 16, bear_count: 4, total: 20, bull_pct: 0.8, bear_pct: 0.2, strength: 0.8, is_top_bull: true, is_top_bear: false, scan_time: '11:15' },
  ...
  { sector: 'NBFC', bull_count: 9, bear_count: 6, total: 15, bull_pct: 0.6, bear_pct: 0.4, strength: 0.6, is_top_bull: false, is_top_bear: false, scan_time: '11:15' },
  ...
  { sector: 'INFRA', bull_count: 7, bear_count: 7, total: 14, bull_pct: 0.5, bear_pct: 0.5, strength: 0.5, is_top_bull: false, is_top_bear: false, scan_time: '11:15' },
  { sector: 'INSURANCE', bull_count: 4, bear_count: 6, total: 10, bull_pct: 0.4, bear_pct: 0.6, strength: 0.6, is_top_bull: false, is_top_bear: false, scan_time: '11:15' },
...
const API_SECRET = typeof process !== 'undefined' ? process.env?.NEXT_PUBLIC_RADAR_SECRET || '' : '';
...
export default function SectorSentiment() {
  ...
  const load = useCallback(async () => {
    if (!API_BASE || !API_SECRET) return;

    setLoading(true);
    try {
      const date = new Date().toISOString().split('T')[0];
      const url = `${API_BASE}?route=sector-sentiment&date=${date}&secret=${API_SECRET}`;
  ...
  return (
    <div style={{ fontFamily: 'var(--font-sans)', padding: '0.25rem 0 1rem' }}>
      ...
      >
        <div>
          <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--color-text-primary)' }}>Sector Sentiment</div>
      ...
      {bullSectors.length > 0 && (
        <section style={{ marginBottom: '1.25rem' }}>
          ...
          {bullSectors.map((item, i) => (
            <SectorRow key={item.sector} item={item} type="BULL" animated={animated} rank={i} />
          ))}
        </section>
      )}
      ...
      {bearSectors.length > 0 && (
        <section>
          ...
          {bearSectors.map((item, i) => (
            <SectorRow key={item.sector} item={item} type="BEAR" animated={animated} rank={i} />
          ))}
        </section>
      )}
      ...
      >
        ...
        {updatedAt && <span>refreshed {updatedAt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>}
...
function SectorRow({ item, type, animated }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, height: 18, background: barBg, borderRadius: 3, overflow: 'hidden', position: 'relative' }}>
        <div
          style={{
            position: 'absolute',
            top: 0,
...
function Pill({ color, label, count }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 1 }}>
      ...
      <span style={{ fontSize: 10, color: 'var(--color-text-tertiary)', fontFamily: 'var(--font-mono)' }}>{count}</span>
    </div>
  );
}