...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/ai-signals', icon: Cpu, label: 'AI Top Picks' },
    ],
  },
];
    ...
    >
      ...
      >
        <div className="flex items-center gap-2.5">
          <div className="relative w-7 h-7 flex items-center justify-center shrink-0">
            <div
              className="absolute inset-0 rounded-full border-2 opacity-20 group-hover:opacity-40 transition-opacity"
            ...
            <div
              className="absolute inset-0 rounded-full border-r-2 rotate-[40deg] opacity-60"
              style={{ borderColor: 'var(--color-brand-accent)' }}
            />
          </div>
          <div>
            ...
            <div
              className="font-bold tracking-[0.3em] text-[10px] leading-none mt-0.5"
        ...
        <div className="font-mono text-[9px] tracking-widest mt-2" style={{ color: 'var(--color-brand-muted)' }}>
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
        {NAV_GROUPS.map((group) => (
          <div key={group.label}>
            <div
              className="font-mono text-[9px] tracking-[0.22em] px-2 mb-2"
                ...
                return (
                  <li key={href}>
                    <Link
                      ...
                      onMouseEnter={(e) => {
                        if (!isActive) (e.currentTarget as HTMLElement).style.color = 'var(--color-brand-text)';
                      }}
                      onMouseLeave={(e) => {
                        if (!isActive) (e.currentTarget as HTMLElement).style.color = 'var(--color-brand-muted)';
                      }}
                    >
                      <Icon size={14} />
                      {label}
      ...
      <div className="px-4 py-4 border-t" style={{ borderColor: 'var(--color-brand-border)' }}>
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[9px] tracking-[0.2em]" style={{ color: 'var(--color-brand-muted)' }}>
            LIVE · NSE · F&O
          </span>
        </div>
        <div
