...
function Board({ side, rows, sort, setSort, brokeOnly, setBrokeOnly }: { side: Side; rows: Row[]; sort: { key: SortKey; strongFirst: boolean }; setSort: (key: SortKey) => void; brokeOnly: boolean; setBrokeOnly: (next: boolean) => void }) {
  const ranked = rows.filter((row) => !brokeOnly || Boolean(row.brk && row.bt && row.ls)).sort((a, b) => {
    if (sort.key === "move") {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col">
      <div className="flex items-baseline gap-3 px-1 pb-2">
        ...
        <button onClick={() => setBrokeOnly(!brokeOnly)} className="ml-auto rounded border px-2 py-1 text-[10px] tracking-wider transition" style={{ borderColor: brokeOnly ? tint[side] : "rgba(255,255,255,0.1)", color: brokeOnly ? tint[side] : "rgba(255,255,255,0.5)", background: brokeOnly ? `${tint[side]}11` : "transparent" }}>{brokeOnly ? "BROKE ONLY" : "ALL BREAKS"}</button>
      </div>
...
export default function RustyClient({ snaps }: { snaps: Snap[] }) {
  ...
  return <div className="flex h-full flex-col gap-3 px-4 pb-4 pt-3">
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4 lg:flex-row"><Board side="bull" rows={snap?.bull ?? []} sort={bullSort} setSort={(key) => changeSort("bull", key)} brokeOnly={bullBrokeOnly} setBrokeOnly={setBullBrokeOnly} /><Board side="bear" rows={snap?.bear ?? []} sort={bearSort} setSort={(key) => changeSort("bear", key)} brokeOnly={bearBrokeOnly} setBrokeOnly={setBearBrokeOnly} /></div>
  </div>;
}
