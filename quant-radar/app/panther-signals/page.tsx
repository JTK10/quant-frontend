...
async function getPantherSignals(dateStr: string): Promise<PantherRow[]> {
  try {
    ...
    if (!response.ok) {
      ...
      return [{ name: `API ERR: ${response.status} ${errText.substring(0, 50)}`, source: "panther", side: "NEUTRAL", ts: 0, time: "00:00:00", k_level: "K0", entry: 0, surge: 0, win60s_cr: 0, spread_bps: 0, spread_vs_norm: 0, instrument_key: "", Chart: "" }];
    }
    return response.json();
  } catch (err) {
    console.error("Error fetching Panther Signals:", err);
    return [{ name: `FETCH ERR: ${String(err)}`, source: "panther", side: "NEUTRAL", ts: 0, time: "00:00:00", k_level: "K0", entry: 0, surge: 0, win60s_cr: 0, spread_bps: 0, spread_vs_norm: 0, instrument_key: "", Chart: "" }];
  }
}
