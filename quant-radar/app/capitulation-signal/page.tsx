...
export default async function CapitulationSignalPage({ searchParams }: { searchParams: DateSearchParams }) {
  const dateStr = await resolveDate(searchParams);
  const signals = await getCapitulationSignals(dateStr);
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-hidden relative">
        <CapitulationClient signals={signals} />
      </div>
    </div>
  );
}
