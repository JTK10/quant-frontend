...
export default async function SniperSignalPage({ searchParams }: { searchParams: DateSearchParams }) {
  const dateStr = await resolveDate(searchParams);
  const signals = await getSniperSignals(dateStr);

  const bulls = signals.filter(s => s.Direction === "LONG");
  const bears = signals.filter(s => s.Direction === "SHORT");
  
  // Decide overall bias quickly based on count logic or default it
